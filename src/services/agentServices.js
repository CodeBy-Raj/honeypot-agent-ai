...
const generateReply = async (userMessage, conversationHistory = [], persona = null, goal = "engage") => {
  try {
    let history = [];
    ...
    if (goal) {
       ...
       if (goal === "lure_payment_details") {
           systemInstruction += " Pretend you are ready to pay but the link isn't working, or you need the UPI ID directly.";
       } else if (goal === "stall_and_validate") {
           systemInstruction += " Act confused. Ask for the payment details again. Say your internet is slow. Waste their time.";
       }
    }

    // Since we can't easily change the system prompt of an instantiated chat in this SDK version simply, 
    // we will prepend it to the history or the first message, 
    // OR just instantiate a new model config if needed. 
    // But `model.startChat` supports `systemInstruction` in newer versions.
    // Let's prepend to the prompt for robustness.
